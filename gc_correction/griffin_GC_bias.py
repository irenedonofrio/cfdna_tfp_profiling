...
import argparse
import sys
# matplotlib is imported lazily, only when plots are requested (see make_plots)
...
parser.add_argument('--no_plots', action='store_true', help='skip generating the QC PDF plots (faster). The GC_bias.txt output is unchanged.')
...
os.makedirs(out_dir +'/GC_bias/', exist_ok=True)
if make_plots:
    os.makedirs(out_dir +'/GC_plots/', exist_ok=True)
...
for i in range(size_range[0],size_range[1]+1):
    ...
    GC_freq_parts.append(current_data)
GC_freq = pd.concat(GC_freq_parts, ignore_index=True)
...
for length in new_df['length'].unique():
    ...
    new_df2_parts.append(current)
    
    #print(length,len(current),np.nanmean(current['smoothed_GC_bias']))

new_df2 = pd.concat(new_df2_parts, ignore_index=True)
new_df = new_df2
...
# ---- plotting (optional; controlled by --no_plots) ----
# The GC_bias.txt above is the file the downstream profiling step needs.
# The PDFs below are QC visuals only, so they can be skipped for speed.
if not make_plots:
    print('done (GC_bias.txt written; plots skipped via --no_plots)')
    sys.exit(0)

import matplotlib
matplotlib.use('Agg')  # headless-safe backend (no display needed on a cluster/container)
